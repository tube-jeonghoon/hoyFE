...
const Workspace = () => {
  ...
  // 동적라우트 워크스페이스 아이디 받아오는 부분
  useEffect(() => {
    if (router.query.id) {
      setWorkspaceId(router.query.id as string);
    }
  }, [router.query.id]);
  ...
  const { data: todosData } = useQuery(
    ['todos', workspaceId, currentDate.formatDate],
    async () => {
      const accessToken = Cookies.get('ACCESS_KEY');
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/workspace/${workspaceId}/tasks?date=${currentDate.formatDate}`,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
          },
  ...
  const toggleTaskCompletion = async (taskId: number) => {
    try {
      ...
      const res = await axios.put(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/workspace/${workspaceId}/tasks/${taskId}/status`,
        {},
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
  ...
  const updateTaskDate = async ({
    ...
    try {
      const response = await axios.put(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/workspace/${workspaceId}/tasks/${taskId}/drag`,
        { date: newDate },
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
